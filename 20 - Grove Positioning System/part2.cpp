...
std::vector<NodePtr> parseInput(std::istream & is)
{
    ...
    for (String line; std::getline(is, line);) {
        nodes.emplace_back(std::make_shared<Node>(std::stoi(line) * key));
    }
...
void decrypt(std::vector<NodePtr> & nodes)
{
    ...
    for (const auto & node : nodes) {
        ...
        // Int steps = std::abs(node->value() % (numNodes - 1));
...
Int score(const std::vector<NodePtr> & nodes)
{
    ...
    for (Int i = 0; i < 3; ++i) {
        ...
        std::cout << "score[" << i << "] = " << node->value() << std::endl;
        score += node->value();
    }
