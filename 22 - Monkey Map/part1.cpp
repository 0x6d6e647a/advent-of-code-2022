...
Direction oppositeDirection(const Direction & direction)
{
    ...
    std::cerr << "invalid direction for opposite" << std::endl;
    std::terminate();
}
...
CoordinatePair ghostWalk(const Position & playerPosition, const BoardMap & boardMap)
{
    CoordinatePair playerCoord{playerPosition.first};
    Direction oppDir{oppositeDirection(playerPosition.second)};

    while (true) {
        CoordinatePair nextCoord{playerCoord + directionToForceVector(oppDir)};

        if (!boardMap.isWall(nextCoord) && !boardMap.isTile(nextCoord)) {
            break;
        }
    ...
    // Wrapped around to wall, just return original position.
    if (boardMap.isWall(playerCoord)) {
        ...
        if (std::holds_alternative<Int>(step)) {
            ...
            for (int moveIndex = 0; moveIndex < numMoves; ++moveIndex) {
                ...
                if (boardMap.isWall(nextCoord)) {
                    ...
                    nextCoord = ghostWalk({playerCoord, playerDirection}, boardMap);

                    if (nextCoord == playerCoord) {
                        break;
                    }
